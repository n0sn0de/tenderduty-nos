...
type ChainConfig struct {
	...
	// ExtraInfo will be appended to alert data. This is useful for PagerDuty when multiple Seer instances
	// are watching the same chain; it helps identify which instance sent the alert.
	ExtraInfo string `yaml:"extra_info"` // FIXME not used yet!
	// Alerts defines the types of alerts to send for this chain.
	Alerts AlertConfig `yaml:"alerts"`
	// PublicFallback determines if Seer should attempt public RPC endpoints when no configured endpoint is available.
...
type AlertConfig struct {
	...
	// AlertIfInactive decides if Seer sends an alert when the validator is not in the active set.
