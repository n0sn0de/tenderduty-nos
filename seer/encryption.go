...
func encrypt(plainText []byte, password string) (encryptedConfig []byte, err error) {
	...
	if padLen > 0 {
		// #nosec G115 -- AES block sizes are at most 16 bytes, so this conversion is bounded.
...
func decrypt(encodedFile []byte, password string) (plainText []byte, err error) {
	...
	decoded := make([]byte, base64.StdEncoding.DecodedLen(len(encodedFile)))
	size, err := base64.StdEncoding.Decode(decoded, encodedFile)
	if err != nil {
		return nil, err
	}
	cipherText := decoded[:size]
	minimumSize := idKeySize + ivSize + aes.BlockSize + macSize
	cipherBytes := size - idKeySize - ivSize - macSize
	if size < minimumSize || cipherBytes%aes.BlockSize != 0 {
		return nil, errors.New("ciphertext has an invalid length")
	}
	...
	if len(plainText) == 0 {
		return nil, errors.New("plaintext was empty")
	}
	...
	// #nosec G115 -- padLen is bounded to the AES block size above.
	if !bytes.Equal(plainText[len(plainText)-padLen:], bytes.Repeat([]byte{byte(padLen)}, padLen)) {
		return nil, errors.New("plaintext has invalid padding")
	}
